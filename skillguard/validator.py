...
DANGEROUS_PATTERNS: list[tuple[str, str, Severity]] = [
    ...
    # 恶意音频窃取
    ("SEC-516", r"\b(?:arecord|ffmpeg)\s+[^\n]*(?:-d|--duration)\s+\d+", Severity.WARNING),
    # 危险位置追踪
    ("SEC-517", r"\bgeoiplookup\s+|curl\s+.*\S*(?:geoip|location)\S*\s*$", Severity.INFO),
    # 恶意网络钓鱼工具
    ("SEC-518", r"\b(?:setoolkit|beef-xss|socialfish)\s+", Severity.WARNING),
    # 危险远程控制服务
    ...
    # 恶意数据掩码绕过
    ("SEC-531", r"\b(?:obfuscate|masker|cloak)\s+[^\n]*(?:-o|--output)\s+", Severity.INFO),
    # 危险内核符号导出
    ("SEC-532", r"\b(?:nm|objdump)\s+[^\n]*(?:/boot|/usr/lib/modules).*\.(?:ko|o)\b", Severity.INFO),
    # 恶意硬件调试器
    ("SEC-533", r"\b(?:openocd|avrdude|st-flash)\s+[^\n]*(?:-c|--command)\s+", Severity.WARNING),
    # 危险文件系统挂载读写
    ...
    # 恶意数据仓库外传
    ("SEC-566", r"\b(?:mysqldump|pg_dump)\s+|sqlite3\s+[^\n]*\.dump\b", Severity.WARNING),
    # 危险文件系统只读绕过
    ("SEC-567", r"\b(?:mount|remount)\s+[^\n]*(?:-o|--options)\s+\S*rw\S*\s+/", Severity.WARNING),
    # 恶意系统信息泄露
    ("SEC-568", r"\b(?:lsblk|blkid)\s+[^\n]*(?:-o|--output)\s+\S+", Severity.INFO),
    # 危险防火墙静默
    ("SEC-569", r"\b(?:iptables|nft)\s+[^\n]*(?:-F|-X|flush)\b", Severity.WARNING),
    # 恶意网络接口配置
    ("SEC-570", r"\b(?:ip|ifconfig)\s+.*\s(?:add|addr)\s+\S+\s+dev\s+\S+", Severity.INFO),
    # 恶意系统调用注入
    ("SEC-571", r"\b(?:strace|ltrace)\s+[^\n]*(?:-e|--trace)\s+\S+", Severity.WARNING),
    # 危险安全策略覆盖
    ("SEC-572", r"\b(?:setsebool|semanage)\s+[^\n]*(?:-P|-m)\s+", Severity.WARNING),
    # 恶意文件监控
    ("SEC-573", r"\b(?:inotifywait|auditctl)\s+[^\n]*(?:-m|-w)\s+\S+", Severity.WARNING),
    # 危险网络流量镜像
    ...
    # 恶意负载持久化
    ("SEC-581", r"\b(?:cp|mv)\s+\S+\s+/usr/(?:local/)?bin/\S+", Severity.WARNING),
    # 危险系统命令替换
    ("SEC-582", r"\b(?:ln|cp|mv)\s+\S+\s+/bin/(?:ls|ps|top|bash|sh)\s*$", Severity.ERROR),
    # 恶意核心转储利用
    ("SEC-583", r"\b(?:ulimit|prlimit)\s+[^\n]*(?:-c|--core)\s+\S+", Severity.INFO),
    # 危险沙箱网络逃逸
    ...
    # 恶意系统状态欺骗
    ("SEC-636", r"\b(?:touch|stat)\s+[^\n]*\S+\.(?:pid|lock)", Severity.INFO),
    # 危险服务状态伪造
    ("SEC-637", r"\b(?:systemctl|service)\s+[^\n]*(?:-f|--force)\s+[^\n]*(?:start|stop|restart)\s+", Severity.WARNING),
    # 恶意进程伪装
    ("SEC-638", r"\bexec\s+-a\s+\S+\s+\S+|\b(?:prctl|setproctitle)\s+", Severity.WARNING),
    # 危险系统日志伪造
